...
$title = isset($page_title)
    ...
    ? ($page_title . ' — ' . t('site_title_suffix'))

    : (t('site_title_suffix') . ' | ' . mb_substr(t('site_desc'), 0, 72));
...
$assetBase = rtrim(SITE_URL, '/');

$logoUrl   = $assetBase . '/assets/img/logo.png';
...
<meta name="apple-mobile-web-app-title" content="<?= e($lang === 'ar' ? 'foot-boll' : 'foot-boll') ?>">

<?php

seo_head([

    'title'       => isset($page_title) ? ($page_title . ' — ' . t('site_title_suffix')) : t('site_title_suffix'),
...
<?php require __DIR__ . '/gtm-body.php'; ?>
  ...
  <div class="wrap header-shell">
    ...
    <div class="header-top">
      ...
      <a class="brand" href="<?= e(url('index.php')) ?>">
        ...
        <img class="brand-logo" src="<?= e($logoUrl) ?>" alt="foot-boll.com" width="160" height="44" loading="eager">
...
<?php
          ...
          if (defined('DB_ENABLED') && DB_ENABLED):
              ...
              $authUserM = class_exists('Auth') ? Auth::user() : null;

              if ($authUserM !== null): ?>

          <a class="nav-user" href="<?= e(url('logout.php')) ?>"><?= e(t('logout')) ?></a>

<?php         else: ?>

          <a href="<?= e(url('login.php')) ?>"><?= e(t('login')) ?></a>
...
?>
          ...
          <a class="lang-switch" href="<?= e(url($current, ['lang' => $code])) ?>"><?= e($label) ?></a>
